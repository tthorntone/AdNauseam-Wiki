...
First make sure that you are working with the correct commit (check the hash) or release candidate number, then perform each test on Firefox, Chrome, Opera and Edge. Check the console for the background.html page as well as the browser console for UI pages throughout the process, report any error/warning messages.
...
- [ ] Make sure all built files are correctly installing on their respective browsers (generated by `tools/make-artifacts.sh` script)
...
__ADNAUSEAM IS DISABLED ON FIREFOX ANDROID FOR NOW__
