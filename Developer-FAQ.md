...
By default AdNauseam only blocks requests that do not stops ads from being collected (and later clicked). When ’Strict blocking mode’ is enabled, AdNauseam will block ANY request that matches a blocking rule from an enabled third-party list. This means that less ads will be collected and placed in the vault, and later clicked. Thus AdNauseam will be less effective in its primary function of collecting/clicking ads (for example, the majority of Google ads won’t be collected or clicked). Thus for most users 'Strict blocking mode' is NOT recommended. However, if you are primarily interested only in blocking ads, you may see better performance 'Strict blocking mode' enabled, depending on the sites that you visit. You can read more about blocking and hiding rules in AdNauseam at [here](https://github.com/dhowe/AdNauseam/wiki/Developer-FAQ#what-is-the-relationship-between-blocking-and-hiding-rules-in-ublock-and-adn). Please use this mode with caution and make sure that you fully understand the consequences of enabling it. 
