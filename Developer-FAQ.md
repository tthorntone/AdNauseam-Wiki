...
By default AdNauseam only blocks requests that won't stop ads from being collected. When ’Strict blocking mode’ is enabled, AdNauseam will block ANY request that matches a blocking rule from an enabled third-party list. This means that LESS ads will be collected, placed in the vault, and later clicked, and that AdNauseam will be less effective in its primary function (for example, the majority of Google ads won’t be collected or clicked). Thus for most users 'Strict blocking mode' is NOT recommended. However, if you are primarily interested in blocking ads, you may see better performance with 'Strict blocking mode' enabled, depending on the sites that you visit. You can read more about blocking and hiding rules in AdNauseam [here](https://github.com/dhowe/AdNauseam/wiki/Developer-FAQ#what-is-the-relationship-between-blocking-and-hiding-rules-in-ublock-and-adn). In general this mode is only for advanced users with specific use-cases (e.g., testing) - please make sure that you fully understand the consequences before enabling it. 
