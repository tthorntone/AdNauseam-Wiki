...
[Pay-per-Click (PPC)](https://en.wikipedia.org/wiki/Pay_per_click) is a common internet advertising model in which advertisers pay for each click on Ads appearing on a website. 
The cost involved varies widely depending on a number of factors including the type of website the Ad appears on; whether a normal 'display' website, or a 'search' website where the Ads appear based on the user's query. The latter is generally more effective and costs are commonly calculated on the spot through a real-time bidding system. Display ads might use fixed prices or other models to determine cost. The content of the Ad also matters. Depending on these factors, costs per click range from below 1$ to over $50.  As the precise cost generated by clicks is not visible to the client, **AdNauseam calculates an estimate using an average value of $1.58**. The value is taken from [this analysis](https://www.hochmanconsultants.com/cost-of-ppc-advertising/), in which various advertising models and platforms are taken into account.  
