...
[Pay-per-Click (PPC)](https://en.wikipedia.org/wiki/Pay_per_click) is a common internet advertising model in which advertisers pay for each individual click on one of their ads appearing on a website. 
The cost involved varies widely depending on the type of website the Ad appears on; as a generalisation it can be distinguished between 'search websites' where corresponding ads appear based on the user's query, and 'display websites'. The former is more effective and costs are commonly calculated on the spot through a biding system. Pages might use fixed prices or even different models altogether. The content of the Ad also matters. Depending on these factors, costs per click range from below 1$ to over $50. 
As the precise cost generated by clicks is not visible to the client, **AdNauseam calculates an estimate using an average value of $1.58**. The value is taken from [THIS ANALYSIS](https://www.hochmanconsultants.com/cost-of-ppc-advertising/) in which various advertising models and platforms are taken into account.  

- [Additional info on how much Google's Adwords cost](http://www.wordstream.com/blog/ws/2015/05/21/how-much-does-adwords-cost)
- [Informative visualisation of other costs caused by targeted advertising](http://www.nytimes.com/interactive/2015/10/01/business/100000003949287.mobile.html). 
