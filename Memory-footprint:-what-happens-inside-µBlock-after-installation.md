...
The next time you launch µBlock and there is a valid selfie, the load time will be a fraction of the load time when no selfie is available <sup>[1]</sup>, and µBlock's baseline memory footprint will be smaller than when µBlock launches without a selfie available.
